...
export default function RidePage() {
  ...
  const [driverDirections, setDriverDirections] = useState<any>(null);
...
function calculateDriverRoute(driver: any) {
  if (!isLoaded || !window.google) return;
  if (!driver?.lat || !driver?.lng || !pickup) return;
  ...
  setMessage("Enjoy your GlideWay ride.");
    ...
    async function loadNearbyDrivers() {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/drivers/nearby`);
const data = await res.json();
  ...
  setMessage("Google Maps is still loading. Please try again.");
  return;
}
    const service = new window.google.maps.DirectionsService();
  ...
  async function requestRide() {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/rides/request`, {
        method: "POST",
    ...
    if (!isLoaded) {
  return <main style={pageStyle}>Loading map...</main>;
}
